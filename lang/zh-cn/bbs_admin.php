...
return array(
	...
	'locale_unavailable'=>'所选语言不可用或必需的语言资源不完整。',
	...
	'plugin_safe_mode'=>'应用插件安全模式',
	'plugin_safe_mode_marker_path'=>'标记文件路径',
	'plugin_safe_mode_log_path'=>'安全模式日志路径',
	'plugin_safe_mode_latest_error'=>'最近一次插件或编译运行时错误',
	'plugin_safe_mode_exit'=>'退出插件安全模式',
	'plugin_safe_mode_exit_success'=>'安全模式标记已清除，下一次请求将恢复正常插件加载。',
	'plugin_safe_mode_exit_locked'=>'另一个安全模式操作正在执行，请稍后重试。',
	'plugin_safe_mode_exit_clear_failed'=>'一个或多个安全模式标记无法清除，请检查页面显示的路径和文件权限；插件仍保持禁用。',
	'plugin_safe_mode_exit_unlock_failed'=>'安全模式标记已清除，但操作锁释放失败，请刷新并确认状态后再继续。',
	...
	// 插件
	'plugin_dir'=>'插件目录名',
	'plugin_dir_invalid'=>'插件目录名无效。请使用 1 至 64 位字母、数字、下划线或连字符，且首字符不能是连字符。',
	...
	'plugin_setting_path_invalid'=>'插件设置页面路径无效。',
	'plugin_package_path_invalid'=>'插件包路径不可用或不安全，请确认它是可读的本地真实目录且不是符号链接。',
	'plugin_setting_no_output'=>'插件设置页面没有输出内容。该插件可能没有可配置项，或其设置界面与当前版本不兼容。',
	...
	'plugin_being_dependent_cant_disable'=>'不能禁用 {name}，以下插件依赖它：{s}',
	...
	'plugin_state_unavailable'=>'插件状态暂时不可用，请检查运行时锁目录后重试。',
	'plugin_state_storage_readonly'=>'插件状态存储 {file} 为只读。不可变部署中无法安装、启用、禁用或卸载插件；设置页面仍可使用。',
	'plugin_state_storage_readonly_short'=>'插件状态为只读',
	...
	'update_network_ca'=>'TLS 证书信任失败：服务器缺少或无法读取有效的 CA 证书包',
	'update_network_tls'=>'TLS 握手或证书校验失败',
	'update_network_dns'=>'DNS 无法解析 GitHub 或代理地址',
	'update_network_connect'=>'无法连接 GitHub 或代理服务器',
	'update_network_timeout'=>'连接 GitHub 或代理服务器超时',
	'update_network_http'=>'远端服务器返回 HTTP {status}',
	'update_network_redirect'=>'远端重定向不安全或次数过多，已停止请求',
	'update_network_curl'=>'服务器缺少安全在线更新所需的 cURL 能力',
	'update_network_policy'=>'更新地址未通过公共 HTTPS 安全校验',
	'update_invalid_release'=>'GitHub Release 版本号格式异常，已停止更新',
	...
	'update_checksum_mismatch'=>'更新包 SHA256 校验失败，已停止更新',
	'update_checksum_missing_blocked'=>'未找到发布包 SHA256 校验信息，已停止更新',
	'update_checksum_verified'=>'SHA256 校验已通过。',
	'update_checksum_unverified'=>'未找到发布包 SHA256 校验信息，已记录本地 hash。',
