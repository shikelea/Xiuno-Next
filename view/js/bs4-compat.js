...
// 后台插件操作兼容：旧后台主题覆盖 plugin_list.htm 时可能遗漏 data-method="post"
// 只修复明确的插件写操作链接，继续交给核心 bbs.js 的 POST/confirm 处理器执行。
(function() {
    'use strict';
    var actionRe = /(?:^|[?&\/-])plugin-(download|install|enable|disable|unstall|upgrade)-/;
    var classRe = /\b(download|install|enable|disable|unstall|upgrade)\b/;
    ...
    function fixPluginPostLinks(root) {
        root = root || document;
        if (!root || typeof root.querySelectorAll !== 'function') return;
        var links = root.querySelectorAll('a.download,a.install,a.enable,a.disable,a.unstall,a.upgrade,a[href*="plugin-download-"],a[href*="plugin-install-"],a[href*="plugin-enable-"],a[href*="plugin-disable-"],a[href*="plugin-unstall-"],a[href*="plugin-upgrade-"]');
        for (var i = 0; i < links.length; i++) {
            if (!links[i].hasAttribute('data-method') && isPluginWriteLink(links[i])) {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() { fixPluginPostLinks(document); });
    } else {
        fixPluginPostLinks(document);
    }
    if (typeof MutationObserver !== 'undefined') {
        var observer = new MutationObserver(function(mutations) {
            for (var i = 0; i < mutations.length; i++) {
                var nodes = mutations[i].addedNodes;
                for (var j = 0; j < nodes.length; j++) {
                    if (nodes[j].nodeType === 1) fixPluginPostLinks(nodes[j]);
                }
            }
        });
        function startObs() {
...
(function() {
    ...
    if (!jQuery.fn.alert || !jQuery.fn.alert._xnPatched) {
        jQuery.fn.alert = function(message) {
            ...
            if (!jfeedback.length) {
                jfeedback = jQuery('<div class="invalid-feedback"></div>').insertAfter(jthis);
            }
            ...
            if (jQuery.inArray(feedbackId, describedby) === -1) {
                describedby.push(feedbackId);
            }
            jthis.attr('aria-describedby', jQuery.trim(describedby.join(' ')));
