...
if (y > 0) unchecked_push(x, y - 1);
if (x < width - 1) unchecked_push(x + 1, y);
if (y < height - 1) unchecked_push(x, y + 1);
if (x > 0) unchecked_push(x - 1, y);
}
checked_push(x, y);
}
};
...
ctx.drawImage(img, 0, 0, img_width, img_height, dx, dy, width, height);
...
ctx.globalAlpha = 0.3;
ctx.beginPath();
ctx.drawImage(water_img, 0, 0, water_width, water_height, x, y, water_width, water_height);
ctx.closePath();
ctx.save();
}
...
jthis.data('default-text', jthis.html());
...
jthis.attr('data-bs-original-title', message);
try {
