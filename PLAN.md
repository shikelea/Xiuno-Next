...
- [ ] **前端精简**（减法而非加法）：
  ...
  - [x] **前端资产边界文档**：新增 `docs/frontend-assets.md`，记录扫描方式、删除规则和暂留的兼容候选（如 `vue.js`、`popper-utils.js`、`upload.js`）。
  ...
  - **下一步**：用本地插件/主题样本验证 `vue.js`、`popper-utils.js`、`upload.js` 等兼容候选，确认后再进入 HTMX 小范围试点。
...
6. **前端精简审计**：持续使用 `php bin/scan_frontend_assets.php` 检查核心资产引用；下一步结合本地插件/主题样本验证 `vue.js`、`popper-utils.js`、`upload.js` 等兼容候选，再决定是否继续删除或转入 HTMX 小试点。
