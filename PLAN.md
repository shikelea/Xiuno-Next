...
- [x] **真实生态样本兼容审计（阶段六输入已建立）**：
  ...
  - 使用 `php bin/build_ecosystem_matrix.php` 将扫描结果归一化为本地 `tmp/ecosystem_matrix.json` / `tmp/ecosystem_matrix.md`，字段覆盖状态、问题类型、最低 Xiuno Next 版本、workaround、修复归属、缺失 Hook 明细和元数据有效性；生成物不纳入仓库。
