...
生态重建不是终点。长期目标是在插件/主题生态稳定之后，继续用 Xiuno 打造一种如 BMT 般惊艳、原生、优雅的论坛体验：在前端能力受限、不引入重框架的前提下，探索现代浏览器能力、渐进增强、局部交互和高质感主题系统，同时保持 Xiuno 的轻量、极速与可掌控。
...
- [x] **前端精简（主体完成，持续审计）**（减法而非加法）：
  ...
  - [x] **前端安全审查增量**：
    ...
    - 对照 CVE-2018-8942 / CNVD-2018-07560 修复后台站点名存储型 XSS：保存端将 `sitename` 收窄为纯文本，输出端对页面标题、meta 文本和首页站点名统一 HTML 转义；`sitebrief` 保留 HTML 能力但使用白名单清洗。
    - 对照重装类 CNVD 修复安装器防御纵深：移除 `DEBUG == 2` 调试绕过，并在数据库安装 POST 分支增加二次已安装阻断；生产环境仍要求删除或封禁 `install/`。
    - 对照 CVE-2018-15559 复核发帖/编辑路径：普通用户正文仍经文本转义或 `xn_html_safe()`；本轮补上服务端附件列表文件名输出转义。管理员 HTML 帖、历史 `message_fmt` 和插件 hook 后处理列入阶段六生态边界治理。
...
- [ ] **API 持续开发（后续增强项）**（阶段三完成了基础接口，此处扩展和完善）：
  ...
  - 扩展 API 覆盖面：已补只读版块 `forum/list`、`forum/read`、只读标题搜索 `search/thread` 与用户可读帖子列表 `user/threads`；后续继续推进用户资料修改、附件上传、通知、版块管理等。
...
- [ ] **插件与主题兼容矩阵**：
  ...
  - 将历史安全边界纳入矩阵字段：是否改写 `message_fmt`、是否覆盖 header/footer、是否执行动态脚本、是否解析 XML/HTML 片段、是否依赖管理员 HTML 直通能力。
- [ ] **主题开发体系**：
...
- [ ] **文档中心重构**：
  ...
  - 插件/主题安全规范在生态重建阶段正式发布：禁止插件对用户输入使用外部实体 XML 解析；如需解析 XML/HTML，必须禁用 DOCTYPE/ENTITY、使用 `LIBXML_NONET` 或项目认可的安全解析方式，并说明 hook 输出的转义责任。
...
### 阶段八：原生体验探索 (Native Experience Phase)
**目标**：生态稳定后，把 Xiuno 从“可用、兼容、可靠”推进到“惊艳、原生、优雅”，在不牺牲轻量与极速的前提下探索下一代论坛体验。

- [ ] **原生交互体验**：
  - 坚持服务端渲染为底座，使用 HTMX、少量原生 JavaScript、现代 CSS 和浏览器能力探针做渐进增强。
  - 优先建设帖子流、回复、通知、搜索、用户资料、后台管理等高频路径的顺滑体验，而不是先做炫技型组件。
  - 新能力必须可降级：旧浏览器、弱网、禁用脚本时仍能完成核心阅读、发帖、回复和管理流程。
- [ ] **优雅主题系统**：
  - 在主题 API 稳定后，形成一套轻量但高级的默认主题设计语言，支持沉浸式阅读、响应式排版、局部刷新和低干扰通知。
  - 主题能力以语义模板、资源入队、布局片段和能力声明为边界，避免把前端框架绑定进核心。
- [ ] **体验性能一体化**：
  - 所有体验升级都要同时通过性能预算、核心页面基线和真实插件/主题样本验证。
  - 把“惊艳”定义为速度、稳定、手感、信息密度和审美共同成立，而不是单纯动画或视觉复杂度。
...
4. **逻辑审查常态化**：把“用户实际路径能否闭环”作为与代码审查、安全审查同级的检查项，重点覆盖安装/升级/回滚、插件依赖与任务锁、权限与版块可见性、后台状态变更、重复提交、异常早退和提示跳转。能在当前阶段完整修复的缺陷直接修到闭环；需要架构调整的缺陷进入计划并标注风险、优先级和验证方式，不以“暂时能用”为完成标准。
5. **依赖与 PHP 版本矩阵**：以 PHP 8.0+ 为最低运行线，Docker 默认 PHP 8.2，CI 覆盖 8.0/8.2/8.3/8.4/8.5，并在 CI 中执行 Composer validate/install；Dependabot 仅用于每周检查 Composer 与 GitHub Actions 更新，Composer 主版本升级默认忽略并改由人工评估，依赖升级优先保证旧插件兼容。后续需要决定是否提交 `composer.lock` 来固定应用依赖。
6. **社区资料边界**：`开发手册/` 保留为本地参考且不进仓库；Xiuno Next 差异说明迁移到 `docs/`，避免在参考资料目录里继续工作。
7. **前端精简审计**：持续使用 `php bin/scan_frontend_assets.php --samples=plugin` 检查核心资产与本地插件/主题样本引用；当前扫描已无疑似无用资产，后续只在删除或替换资源前运行。
8. **生态样本兼容矩阵**：已用本地插件/主题样本库形成初步问题分类，并沉淀为矩阵字段（状态、问题类型、最低 Xiuno Next 版本、workaround、修复归属）；本地兼容测试脚本和生成报告不纳入仓库，下一步优先处理 `conf.json` 元数据损坏、缺失旧 Hook 契约和可重复样本 smoke，不急于发布正式开发文档。
9. **原生插件与主题预览规范**：先维护状态说明和 Hook 索引，再补 `plugin.json` 草案、主题 API 草案、CLI 原生模板和插件/主题 smoke test。
10. **数据库层长期治理**：短期继续收敛直接 SQL 拼接点，长期把高风险写路径迁移到兼容旧插件的预处理语句层；在迁移完成前，不宣称 SQL 注入问题已被彻底解决。
