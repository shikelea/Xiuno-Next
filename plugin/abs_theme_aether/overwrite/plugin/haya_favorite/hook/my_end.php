...
elseif ($action == 'favorite') {
	...
	if ($method == 'GET') {
		...
		$tid = param('tid');
		if (!$user) {
			message(1, lang('haya_favorite_user_favorite_error_tip'));
		}
		...
		$available_positions = [];
		if(!empty($subaction)) {
			$available_positions = array('thread_plugin_body','thread_S3_Footer');
		}
		...
		if ($subaction == 'create') {
			...
			if (!empty($haya_check_favorite)) {
				message(1, lang('haya_favorite_user_have_favorite_tip'));
			}
			$_component = 'thread';
			...
			if($IS_HTMX) {
				$thread['favorites'] = max(0, $haya_favorite_count);
				...
				header('HX-Trigger-After-Settle: ' . json_encode([
					'showSnackBar' => [
						...
						'subtitle' => '',
						'content' => lang('haya_favorite_user_favorite_success_tip'),
						'delay'   => 3000
					]
				], JSON_FORCE_OBJECT));
				include _include( APP_PATH .'plugin/abs_theme_aether/template_parts_plugin/haya_favorite/btn_favorite_' . $_component . '__' . $position . '.htm');
				die;
			} else {
			...
			if (empty($haya_check_favorite)) {
				message(1, lang('haya_favorite_user_no_favorite_error_tip'));
			}
			...
			if($IS_HTMX) {
				...
				header('HX-Trigger-After-Settle: ' . json_encode([
					'showSnackBar' => [
						'type'    => 'success',
						'title'   => lang('tips_title'),
						'content' => lang('haya_favorite_user_delete_favorite_success_tip'),
						'delay'   => 3000
					]
				], JSON_FORCE_OBJECT));
				include _include( APP_PATH .'plugin/abs_theme_aether/template_parts_plugin/haya_favorite/btn_favorite_' . $_component . '__' . $position . '.htm');
	...
	message(0, jump(lang('haya_favorite_my_favorite') . ' - 正在进入', url('my-favorite'), 2));
	die;
	/*
	$header['title'] = lang('haya_favorite_my_favorite') . " - " . $conf['sitename'];
