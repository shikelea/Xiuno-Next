...
define('SEARCH_RANGE_THREAD', 1);
define('SEARCH_RANGE_POST', 0);
define('SEARCH_RANGE_USER', 3);
...
$page = param(3, 1);
*/
$range = param('range', SEARCH_RANGE_THREAD);
$page = param('page', 1);
...
if ($keyword) {
	...
	// TEST By XHSS
	if ($range == 3) {
		$userslist = db_sql_find("SELECT * FROM bbs_user WHERE username LIKE '%$keyword_decode%' LIMIT 50;");
		$userslist = arrlist_multisort($userslist, 'uid', FALSE);
	}
}
