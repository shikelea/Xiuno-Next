...
if ($action == 'cPay') {
	...
	if (!$content_pay) {
		...
		switch ($thread['content_buy_type']) {
			case '1':
				$operation_credit_area = 'credits';
				break;
			case '2':
				$operation_credit_area = 'golds';
				break;
			case '3':
				$operation_credit_area = 'rmbs';
				break;
			default:
				$operation_credit_area = 'rmbs';
				break;
		...
		if ($thread['content_buy'] != 0 && $user[$operation_credit_area] < $thread['content_buy']) {
			message(-3, lang('credit_no_enough') . ' (' . $thread['content_buy_type'] . ')');
			die;
		}
		...
		if ($uid) {
			db_insert('user_pay', array('uid' => $uid, 'status' => 1, 'num' => $thread['content_buy'], 'type' => '4', 'credit_type' => $thread['content_buy_type'], 'code' => $thread['tid'] . ',' . $thread['subject'], 'time' => time()));
		}

		header('HX-Trigger-After-Settle: ' . json_encode(['closeModal' => true,], JSON_FORCE_OBJECT));
