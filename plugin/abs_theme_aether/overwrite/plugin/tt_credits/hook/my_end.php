    ...
    elseif ($action == 'credits') {
        ...
        elseif ($method == 'POST') {
            ...
            if ($op == 'n') {
                ...
                if ($e_rmb < $min) {
                    message(-1, '最低兑换金额：¥' . ($min / 100.0) . '，您兑换的金额不足。');
                    die();
                }
                if ($e_rmb <= 0) {
                    message(-1, '请输入兑换金额。');
                ...
                if ($my_rmbs < $e_rmb) {
                    message(-1, lang('credit_no_enough'));
                    die();
                }
                if (empty($uid) || empty($e_rmb)) {
                    message(-1, '请输入兑换金额。');
                    die();
                }
                ...
                $set = setting_get('tt_credits');
                $e_golds = param('e_golds_c',0);
                $my_golds = $user['golds'];
                $my_rmbs = $user['rmbs'];
                $min = $set['min'];
                $e_golds_raw = $e_golds;
                $e_golds *= $set['exchange_c'];
                if (empty($uid) || empty($e_golds)) {
                    message(-1, "ERROR");
                    die();
                }
                if ($e_golds < $min * $set['exchange_c']) {
                    message(-1, '最低兑换金币：' . ($min * $set['exchange_c']) . '，您兑换的金额不足。');
                    die();
                }
                if ($e_golds <= 0) {
                    message(-1, '请输入兑换金币。');
                ...
                $to_num = param('trans_num',0);
                $credits_type = param('trans_credits',0);
                ...
                if (empty($credits_type)) {
                    message(-1, "请选择转账积分类型。");
                    die();
                }
                if (db_count('user', array('username' => $to_username)) <= 0) {
                    message(-1, "用户不存在!");
                    die();
                }
                ...
                if ($user[$credits_name] < $to_num) {
                    message(-1, "您的余额不足,请充值!");
                    die();
                }
