/** 
* @type {Object}
* @description xiuno.js 封装了部分 PHP 常用的函数，便于代码移植和使用。
*/
var xn = {}; // 避免冲突，自己的命名空间。

// 确定浏览器类型；针对国内的山寨套壳浏览器检测不准确
// “all”已弃用。lib.dom.d.ts(6791, 8): 该声明曾在此处标记为已弃用。
xn.is_ie = (!!document.all) ? true : false;// ie 6 7 8 9
...
/**
 * @description 对字符串中的特殊字符进行转义，防止 XSS 攻击。
...
xn.json_decode = function (s) {
    ...
    try {
        ...
        console.error('JSON parse error:', e.message, s.substring(0, 100) + (s.length > 100 ? '...' : ''));
        return null;
    }
};
...
xn.str_replace = function (s, d, str) { 
    // 安全检查：如果 s 已经是正则表达式，直接使用
    if (s instanceof RegExp) {
        return str.replace(s, d);
    }
    
    // 安全检查：如果 s 以 / 开头和 / 结尾，可能是正则表达式字符串
    if (typeof s === 'string' && s.startsWith('/') && s.lastIndexOf('/') > 0) {
        try {
            ...
            console.warn('正则表达式解析失败，回退到字符串替换:', s);
            // 回退到字符串替换
...
/**
 * @description 比较两个数组的差异，在 arr1 之中，但是不在 arr2 中。返回差异结果集的新数组，
 * @param {Array} arr1 数组1
 * @param {Array} arr2 数组2
 * @returns {Array} 差异结果集的新数组
 */
...
/**
 * @description 对数组进行过滤，返回符合条件的新数组
 * @param {Array} arr 要过滤的数组
 * @param {Function} callback 过滤函数，返回 true 表示保留，返回 false 表示过滤
 * @returns {Array} 符合条件的新数组
 * 
 * @example 
 * // 第一种用法：
 * var arr = [0,1,2,3];
...
$.pdata = function (key, value) {
	// TODO 使用原生JS实现我的功能，看上去是localStorage的包裹函数，存储JSON数据
	console.warn('[DEPRECATED] 使用原生JS实现我的功能；本函数的本质是操作localStorage存取JSON数据');
	var r = '';
...
$.cookie = function (name, value, time, path) {
	// TODO 使用原生JS实现我的功能
	console.warn('[DEPRECATED] 使用原生JS实现我的功能');
	if (typeof value != 'undefined') {
		if (value === null) {
			var value = '';
			var time = -1;
...
$.xget = function (url, callback, retry) {
	// TODO 使用原生JS XHR或Fetch实现我的功能
	console.warn('[DEPRECATED] 使用原生JS XHR或Fetch实现我的功能');
...
$.fn.base64_encode_file = function (width, height, action) {
	...
	console.warn('[DEPRECATED] 使用base64会让传输的数据量增加大约三倍，如果该文件是通过enctype=application/x-www-form-urlencoded，用POST方式传输到后端，请改造后端，让后端接受enctype=multipart/form-data');
...
xn.upload_file = function (file, upload_url, postdata, complete_callback, progress_callback, thumb_callback) {
	// TODO 使用原生JS XHR或Fetch实现我的功能；该函数被附件上传和头像上传使用，无法脱身
	console.warn('若条件允许，请使用正常一些的表单上传方式，如在表单中使用enctype=multipart/form-data，与XHR或Fetch发起请求。xn.upload_file会将文件进行base64编码，然后通过enctype=application/x-www-form-urlencoded传输文件，这会导致大文件上传失败');
	postdata = postdata || {};
...
$.fn.xn_position = function (jfloat, pos, offset) {
	console.warn('[DEPRECATED] 你在使用Xiuno BBS 3.0时期遗留的功能，请换用其他方式实现');
};
...
console.log('xiuno.js loaded; Vanilla JS is Recommended');