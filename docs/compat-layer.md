...
```
┌─────────────────────────────────────────────┐
│  第三层：主题 API（核心对主题的支持）          │  model/misc.func.php
│  message() HTMX · url() 扩展 · 资源注册       │
├─────────────────────────────────────────────┤
│  第二层：前端兼容（CSS + JS）                  │  view/css/bs4-compat.css
│  BS4→BS5 类映射 · 资源 404 降级 · Modal 代理   │  view/js/bs4-compat.js
├─────────────────────────────────────────────┤
│  第一层：运行时兼容（PHP）                     │  xiunophp/php8_compat.php
...
**加载方式**: 在 `header.inc.htm` 中紧跟 `bootstrap.css` 之后加载。
...
**加载方式**: 在 `footer.inc.htm` 中于 `xiuno.js` 之后加载。
...
扫描页面中所有 `<form>` 表单，自动注入 `_token` 隐藏字段。

#### BS4 Modal API 代理

将 jQuery 的 `.modal('show')` / `.modal('hide')` / `.modal('toggle')` 代理到 BS5 的 `bootstrap.Modal` 实例。
...
> **注意**: 覆盖了 `header.inc.htm` / `footer.inc.htm` 的主题（如 aether）需要在自己的模板中调用这两个函数，或使用自己的资源管理机制。
