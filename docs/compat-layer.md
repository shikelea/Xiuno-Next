...
- `message()` now injects hidden `_token` fields into same-site POST forms embedded inside HTML message fragments. External form actions are skipped to avoid token disclosure. This covers legacy install wizards that return a form through AJAX JSON instead of rendering a full page where the normal `<head>` injector and DOM observer can run first.
- Same-origin plugin install links now use a full-page POST fallback with `_token` instead of the generic AJAX-reload handler. Legacy installers that return an HTML setup wizard through `message(0, ...)` can render normally, while enable/disable/uninstall/upgrade links keep the AJAX flow.
