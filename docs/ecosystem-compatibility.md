...
The Discuz-to-Xiuno converter post pass now has a repository guard for first-post detection. `dx_to_xn4.php`, `dx32_to_xn4.php`, and `dx34_to_xn4.php` fetch the current post's source thread by `tid` before calculating `isfirst`, instead of relying on stale `$thread` state left over from the previous thread loop. This closes the first concrete source-version converter defect found while preparing real conversion fixtures.
