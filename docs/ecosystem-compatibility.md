...
`bin/check_ecosystem_dependency_family_smoke.php` turns local-sample dependency families into repository fixtures without committing third-party packages: `huux_notice -> ax_notice_sx/ob_feedback/till_quick_at/tt_medal`, `abs_themeacp_stately -> abs_theme_stately`, and `tt_credits -> tt_gift/tt_medal/tt_offer/tt_redpacket`. It keeps plugin and theme dependency metadata in the same compatibility track, including satisfied, downloaded-but-not-installed, disabled, missing, version-low, and reverse-dependency cases.
